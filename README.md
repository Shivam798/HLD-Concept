...
- [ ] **Security & Abuse** — authn vs authz, OAuth2/OIDC and JWT, session design, encryption in
      transit and at rest, secrets, rate limiting and bot defence, PII and compliance
...
- `.build/` holds the generator: `kit.py` (page components), `svgkit.py` + `figs_*.py`
  (diagrams), `vol_*.py` (one file per volume), the recovered Segoe UI subsets plus Selawik as
