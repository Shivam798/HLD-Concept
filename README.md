...
| 8 | [Rate Limiting](Rate-Limiting-System-Design-Guide.pdf) | 25 | Rate limit vs throttle vs quota vs load shedding, all five algorithms with a diagram each (fixed window and its boundary burst, sliding log, sliding counter, token bucket, leaky bucket in both forms), all five run side by side on one simulated trace, distributed limiting — local vs central vs gossip, the counter race and the Lua fix, fail open vs closed, picking the number, the eight production mistakes, 14 Q&A |
